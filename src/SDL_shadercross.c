...
extern HRESULT __stdcall DxcCreateInstance(REFCLSID rclsid, REFIID riid,
                                           LPVOID *ppv);
#else
...
static void *SDL_ShaderCross_INTERNAL_CompileUsingDXC(
    const SDL_ShaderCross_HLSL_Info *info, bool spirv,
    size_t *size) // filled in with number of bytes of returned buffer
{
  ...
  DxcCreateInstance(&CLSID_DxcCompiler, IID_IDxcCompiler3,
                    (void **)&dxcInstance);
  ...
  entryPointUtf16 = (wchar_t *)SDL_iconv_string(
      "WCHAR_T", "UTF-8", info->entrypoint, entryPointLength);
  ...
  for (Uint32 i = 0; i < numDefineStrings; i += 1) {
    if (info->defines[i].value == NULL) {
      SDL_snprintf(defineString, MAX_DEFINE_STRING_LENGTH, "-D%s=%s",
                   info->defines[i].name, "1");
    } else {
      SDL_snprintf(defineString, MAX_DEFINE_STRING_LENGTH, "-D%s=%s",
                   info->defines[i].name, info->defines[i].value);
    }

    defineStringsUtf16[i] = (wchar_t *)SDL_iconv_string(
        "WCHAR_T", "UTF-8", defineString, MAX_DEFINE_STRING_LENGTH);
  }
  ...
  if (info->include_dir != NULL) {
    ...
    includeDirUtf16 = (wchar_t *)SDL_iconv_string(
        "WCHAR_T", "UTF-8", info->include_dir, includeDirLength);
  ...
  if (info->name) {
    nameUtf16 = (wchar_t *)SDL_iconv_string("WCHAR_T", "UTF-8", info->name,
                                            SDL_utf8strlen(info->name) + 1);
    if (nameUtf16 != NULL) {
      args[argCount++] = nameUtf16; // a bare string inserted into the arguments
                                    // is treated as the source file name
    }
  }
  ...
  ret = dxcInstance->lpVtbl->Compile(dxcInstance, &source, args, argCount,
                                     includeHandler, IID_IDxcResult,
                                     (void **)&dxcResult);
  ...
  if (ret < 0) {
    ...
    if (errors != NULL && errors->lpVtbl->GetBufferSize(errors) != 0) {
      SDL_SetError("HLSL compilation failed: %s",
...
void *SDL_ShaderCross_CompileDXILFromHLSL(const SDL_ShaderCross_HLSL_Info *info,
                                          size_t *size) {
#if SDL_PLATFORM_GDK
  ...
  size_t spirvSize;
  void *spirv = SDL_ShaderCross_CompileSPIRVFromHLSL(info, &spirvSize);
  ...
  void *translatedSource = SDL_ShaderCross_TranspileHLSLFromSPIRV(&spirvInfo);
  ...
  return SDL_ShaderCross_INTERNAL_CompileUsingDXC(&translatedHlslInfo, false,
                                                  size);
#endif
}

void *
SDL_ShaderCross_CompileSPIRVFromHLSL(const SDL_ShaderCross_HLSL_Info *info,
                                     size_t *size) {
  return SDL_ShaderCross_INTERNAL_CompileUsingDXC(info, true, size);
}
...
static ID3DBlob *SDL_ShaderCross_INTERNAL_CompileDXBC(const char *hlslSource,
...
void *SDL_ShaderCross_INTERNAL_CompileDXBCFromHLSL(
    const SDL_ShaderCross_HLSL_Info *info, bool enableRoundtrip,
    size_t *size) // filled in with number of bytes of returned buffer
{
  ...
  if (enableRoundtrip) {
    ...
    transpiledSource = SDL_ShaderCross_TranspileHLSLFromSPIRV(&spirvInfo);
    SDL_free(spirv);
  ...
  ID3DBlob *blob = SDL_ShaderCross_INTERNAL_CompileDXBC(
      ...
      info->entrypoint, shaderProfile, info->enable_debug);
...
void *SDL_ShaderCross_CompileDXBCFromHLSL(
    ...
    size_t *size) // filled in with number of bytes of returned buffer
{
  return SDL_ShaderCross_INTERNAL_CompileDXBCFromHLSL(info, true, size);
}

static void *SDL_ShaderCross_INTERNAL_CreateShaderFromHLSL(
    SDL_GPUDevice *device, const SDL_ShaderCross_HLSL_Info *info,
    SDL_ShaderCross_GraphicsShaderMetadata *metadata) {
  size_t bytecodeSize;

  // We'll go through SPIRV-Cross for all of these to more easily obtain
  // reflection metadata.
  void *spirv = SDL_ShaderCross_CompileSPIRVFromHLSL(info, &bytecodeSize);
  ...
  if (info->shader_stage == SDL_SHADERCROSS_SHADERSTAGE_COMPUTE) {
    result = SDL_ShaderCross_CompileComputePipelineFromSPIRV(device, &spirvInfo,
                                                             (void *)metadata);
  } else {
    result = SDL_ShaderCross_CompileGraphicsShaderFromSPIRV(device, &spirvInfo,
                                                            (void *)metadata);
  }
  SDL_free(spirv);
  return result;
...
SDL_GPUComputePipeline *SDL_ShaderCross_CompileComputePipelineFromHLSL(
  ...
  return (SDL_GPUComputePipeline *)
      SDL_ShaderCross_INTERNAL_CreateShaderFromHLSL(device, info,
                                                    (void *)metadata);
}
...
static SPIRVTranspileContext *SDL_ShaderCross_INTERNAL_TranspileFromSPIRV(
    ...
    const Uint8 *code, size_t codeSize, const char *entrypoint,
  ...
  result = spvc_context_parse_spirv(context, (const SpvId *)code,
                                    codeSize / sizeof(SpvId), &ir);
  if (result < 0) {
  ...
  if (backend == SPVC_BACKEND_HLSL) {
    ...
    spvc_compiler_options_set_uint(
        options,
        SPVC_COMPILER_OPTION_HLSL_FLATTEN_MATRIX_VERTEX_INPUT_SEMANTICS, 1);
    spvc_compiler_options_set_bool(
        options, SPVC_COMPILER_OPTION_HLSL_USE_ENTRY_POINT_NAME,
        !SDL_GetBooleanProperty(
            props, SDL_SHADERCROSS_PROP_SPIRV_PSSL_COMPATIBILITY, false));
    spvc_compiler_options_set_bool(
        options, SPVC_COMPILER_OPTION_HLSL_POINT_SIZE_COMPAT, true);
  }
  ...
  if (backend == SPVC_BACKEND_MSL) {
    const char *_mslVersion = SDL_GetStringProperty(
        props, SDL_SHADERCROSS_PROP_SPIRV_MSL_VERSION, "1.2.0");
  ...
  if (backend == SPVC_BACKEND_MSL &&
    ...
    for (size_t i = 0; i < num_texture_samplers; i += 1) {
      if (!spvc_compiler_has_decoration(compiler, reflected_resources[i].id,
                                        SpvDecorationDescriptorSet) ||
          !spvc_compiler_has_decoration(compiler, reflected_resources[i].id,
      ...
      unsigned int descriptor_set_index = spvc_compiler_get_decoration(
          compiler, reflected_resources[i].id, SpvDecorationDescriptorSet);
      if (!(descriptor_set_index == 0 || descriptor_set_index == 2)) {
        SDL_SetError("%s", "Descriptor set index for graphics texture-sampler "
                           "must be 0 or 2!");
        spvc_context_destroy(context);
        return NULL;
      }

      unsigned int binding_index = spvc_compiler_get_decoration(
          compiler, reflected_resources[i].id, SpvDecorationBinding);
    ...
    for (size_t i = 0; i < num_storage_textures; i += 1) {
      ...
      unsigned int descriptor_set_index = spvc_compiler_get_decoration(
          compiler, reflected_resources[i].id, SpvDecorationDescriptorSet);
      if (!(descriptor_set_index == 0 || descriptor_set_index == 2)) {
        SDL_SetError("%s", "Descriptor set index for graphics storage texture "
                           "must be 0 or 2!");
        spvc_context_destroy(context);
        return NULL;
      }
    ...
    result = spvc_resources_get_resource_list_for_type(
        resources, SPVC_RESOURCE_TYPE_SEPARATE_IMAGE,
        (const spvc_reflected_resource **)&reflected_resources,
        &num_separate_images);
    if (result < 0) {
      SPVC_ERROR(spvc_resources_get_resource_list_for_type);
    ...
    // Storage buffers
    result = spvc_resources_get_resource_list_for_type(
        resources, SPVC_RESOURCE_TYPE_STORAGE_BUFFER,
        (const spvc_reflected_resource **)&reflected_resources,
        &num_storage_buffers);
    if (result < 0) {
      SPVC_ERROR(spvc_resources_get_resource_list_for_type);
    ...
    for (Uint32 i = 0; i < numTextureBindings; i += 1) {
      ...
      result = spvc_compiler_msl_add_resource_binding_2(compiler,
                                                        &textureBindings[i]);
    }
    ...
    for (Uint32 i = 0; i < numBufferBindings; i += 1) {
      if (bufferBindings[i].desc_set == 1 || bufferBindings[i].desc_set == 3) {
        ...
        // Subtract by the texture count because the textures precede the
        // storage buffers in the descriptor set
        bufferBindings[i].msl_buffer =
            uniformBufferCount +
            (bufferBindings[i].binding - numTextureBindings);
      }
  ...
  if (backend == SPVC_BACKEND_MSL &&
    ...
    for (size_t i = 0; i < num_uniform_buffers; i += 1) {
      ...
      if (descriptor_set_index != 2) {
        SDL_SetError(
            "%s", "Descriptor set index for compute uniform buffer must be 2!");
        spvc_context_destroy(context);
        return NULL;
      }
    ...
    for (Uint32 i = 0; i < numTextureBindings; i += 1) {
      if (textureBindings[i].desc_set == 0) {
        ...
        // readwrite textures
        textureBindings[i].msl_texture =
            readonlyTextureCount + textureBindings[i].binding;
        textureBindings[i].msl_sampler =
            readonlyTextureCount + textureBindings[i].binding;
      }
      result = spvc_compiler_msl_add_resource_binding_2(compiler,
                                                        &textureBindings[i]);
    ...
    for (Uint32 i = 0; i < numBufferBindings; i += 1) {
      if (bufferBindings[i].desc_set == 0) {
        // Subtract by the readonly texture count because they precede readonly
        // buffers in the descriptor set
        bufferBindings[i].msl_buffer =
            uniformBufferCount +
            (bufferBindings[i].binding - readonlyTextureCount);
      } else if (bufferBindings[i].desc_set == 1) {
        // Subtract by the readwrite texture count because they precede
        // readwrite buffers in the descriptor set
        bufferBindings[i].msl_buffer =
            uniformBufferCount + readonlyBufferCount +
            (bufferBindings[i].binding - readwriteTextureCount);
      } else {
        // Uniform buffers are alone in the descriptor set
        bufferBindings[i].msl_buffer = bufferBindings[i].binding;
      }
  ...
  if (backend == SPVC_BACKEND_MSL) {
    // Metal doesn't allow a "main" entrypoint, so determine the "cleansed"
    // entrypoint name (e.g. main -> main0 on MSL)
    cleansed_entrypoint = spvc_compiler_get_cleansed_entry_point_name(
        compiler, entrypoint, spvc_compiler_get_execution_model(compiler));
  } else {
...
bool SDL_ShaderCross_ReflectGraphicsSPIRV(
    const Uint8 *code, size_t codeSize,
    SDL_ShaderCross_GraphicsShaderMetadata
        *metadata // filled in with reflected data
) {
  spvc_result result;
  spvc_context context = NULL;
  spvc_parsed_ir ir = NULL;
  ...
  // The number of storage textures is the number of separate images minus the
  // number of samplers.
...
bool SDL_ShaderCross_ReflectComputeSPIRV(
    const Uint8 *bytecode, size_t bytecodeSize,
  ...
  /* Parse the SPIR-V into IR */
  result = spvc_context_parse_spirv(context, (const SpvId *)bytecode,
                                    bytecodeSize / sizeof(SpvId), &ir);
  if (result < 0) {
    SPVC_ERROR(spvc_context_parse_spirv);
    spvc_context_destroy(context);
    return false;
  ...
  for (size_t i = 0; i < num_storage_textures; i += 1) {
    if (!spvc_compiler_has_decoration(compiler, reflected_resources[i].id,
        ...
        !spvc_compiler_has_decoration(compiler, reflected_resources[i].id,
                                      SpvDecorationBinding)) {
      SDL_SetError(
          "%s", "Shader resources must have descriptor set and binding index!");
      spvc_context_destroy(context);
      return false;
    }
  ...
  // Threadcount
  metadata->threadcount_x = spvc_compiler_get_execution_mode_argument_by_index(
      compiler, SpvExecutionModeLocalSize, 0);
  metadata->threadcount_y = spvc_compiler_get_execution_mode_argument_by_index(
      compiler, SpvExecutionModeLocalSize, 1);
  metadata->threadcount_z = spvc_compiler_get_execution_mode_argument_by_index(
      compiler, SpvExecutionModeLocalSize, 2);
...
static void *SDL_ShaderCross_INTERNAL_CompileFromSPIRV(
    SDL_GPUDevice *device, const SDL_ShaderCross_SPIRV_Info *info,
    SDL_GPUShaderFormat targetFormat, void *metadata) {
  spvc_backend backend;
  ...
  SPIRVTranspileContext *transpileContext =
      SDL_ShaderCross_INTERNAL_TranspileFromSPIRV(
          backend, shadermodel, info->shader_stage, info->bytecode,
          info->bytecode_size, info->entrypoint, info->props);
  ...
  if (info->shader_stage == SDL_SHADERCROSS_SHADERSTAGE_COMPUTE) {
    ...
    SDL_ShaderCross_ComputePipelineMetadata *pipelineInfo =
        (SDL_ShaderCross_ComputePipelineMetadata *)metadata;
    SDL_ShaderCross_ReflectComputeSPIRV(info->bytecode, info->bytecode_size,
                                        pipelineInfo);
    ...
    createInfo.num_readonly_storage_textures =
        pipelineInfo->num_readonly_storage_textures;
    createInfo.num_readonly_storage_buffers =
        pipelineInfo->num_readonly_storage_buffers;
    ...
    createInfo.num_readwrite_storage_buffers =
        pipelineInfo->num_readwrite_storage_buffers;
    ...
    if (info->name != NULL) {
      ...
      SDL_SetStringProperty(createInfo.props,
    ...
    if (targetFormat == SDL_GPU_SHADERFORMAT_DXBC) {
      createInfo.code = SDL_ShaderCross_INTERNAL_CompileDXBCFromHLSL(
          &hlslInfo, false, &createInfo.code_size);
    } else if (targetFormat == SDL_GPU_SHADERFORMAT_DXIL) {
      createInfo.code =
          SDL_ShaderCross_CompileDXILFromHLSL(&hlslInfo, &createInfo.code_size);
    } else { // MSL
      createInfo.code = (const Uint8 *)transpileContext->translated_source;
      createInfo.code_size =
          SDL_strlen(transpileContext->translated_source) + 1;
    }
    ...
    if (info->name != NULL) {
      createInfo.props = SDL_CreateProperties();
      SDL_SetStringProperty(createInfo.props,
                            SDL_PROP_GPU_SHADER_CREATE_NAME_STRING, info->name);
    }
...
void *
SDL_ShaderCross_TranspileMSLFromSPIRV(const SDL_ShaderCross_SPIRV_Info *info) {
  SPIRVTranspileContext *context = SDL_ShaderCross_INTERNAL_TranspileFromSPIRV(
      SPVC_BACKEND_MSL, 0, info->shader_stage, info->bytecode,
      info->bytecode_size, info->entrypoint, info->props);
...
void *
SDL_ShaderCross_TranspileHLSLFromSPIRV(const SDL_ShaderCross_SPIRV_Info *info) {
  SPIRVTranspileContext *context = SDL_ShaderCross_INTERNAL_TranspileFromSPIRV(
      SPVC_BACKEND_HLSL,
      SDL_GetBooleanProperty(
          info->props, SDL_SHADERCROSS_PROP_SPIRV_PSSL_COMPATIBILITY, false)
          ? 50
          : 60,
      info->shader_stage, info->bytecode, info->bytecode_size, info->entrypoint,
      info->props);
...
void *
SDL_ShaderCross_CompileDXBCFromSPIRV(const SDL_ShaderCross_SPIRV_Info *info,
                                     size_t *size) {
  SPIRVTranspileContext *context = SDL_ShaderCross_INTERNAL_TranspileFromSPIRV(
      SPVC_BACKEND_HLSL, 51, info->shader_stage, info->bytecode,
      info->bytecode_size, info->entrypoint, info->props);
  ...
  void *result =
      SDL_ShaderCross_INTERNAL_CompileDXBCFromHLSL(&hlslInfo, false, size);
...
void *
SDL_ShaderCross_CompileDXILFromSPIRV(const SDL_ShaderCross_SPIRV_Info *info,
                                     size_t *size) {
#ifndef SDL_SHADERCROSS_DXC
  SDL_SetError("%s", "Shadercross was not compiled with DXC support, cannot "
                     "compile to SPIR-V!");
  return NULL;
#endif

  SPIRVTranspileContext *context = SDL_ShaderCross_INTERNAL_TranspileFromSPIRV(
      SPVC_BACKEND_HLSL, 60, info->shader_stage, info->bytecode,
      info->bytecode_size, info->entrypoint, info->props);
  ...
  void *result = SDL_ShaderCross_CompileDXILFromHLSL(&hlslInfo, size);
...
static void *SDL_ShaderCross_INTERNAL_CreateShaderFromSPIRV(
  ...
  if (shader_formats & SDL_GPU_SHADERFORMAT_SPIRV) {
    if (info->shader_stage == SDL_SHADERCROSS_SHADERSTAGE_COMPUTE) {
      ...
      createInfo.num_readonly_storage_textures =
          pipelineMetadata->num_readonly_storage_textures;
      createInfo.num_readonly_storage_buffers =
          pipelineMetadata->num_readonly_storage_buffers;
      createInfo.num_readwrite_storage_textures =
          pipelineMetadata->num_readwrite_storage_textures;
      createInfo.num_readwrite_storage_buffers =
          pipelineMetadata->num_readwrite_storage_buffers;
      ...
      SDL_GPUComputePipeline *result =
          SDL_CreateGPUComputePipeline(device, &createInfo);
    ...
    if ((shader_formats & SDL_GPU_SHADERFORMAT_DXBC) &&
        SDL_D3DCompile != NULL) {
      ...
      SDL_SetError("SDL_ShaderCross_INTERNAL_CreateShaderFromSPIRV: Unexpected "
                   "SDL_GPUBackend");
      return NULL;
    }
  }

  return SDL_ShaderCross_INTERNAL_CompileFromSPIRV(device, info, format,
                                                   metadata);
}
...
SDL_GPUComputePipeline *SDL_ShaderCross_CompileComputePipelineFromSPIRV(
    ...
    SDL_ShaderCross_ComputePipelineMetadata *metadata) {
  return (SDL_GPUComputePipeline *)
      SDL_ShaderCross_INTERNAL_CreateShaderFromSPIRV(device, info, metadata);
}
...
SDL_GPUShaderFormat SDL_ShaderCross_GetSPIRVShaderFormats(void) {
  /* SPIRV and MSL can always be output as-is with no preprocessing since we
   * require SPIRV-Cross */
